...
const int WS_TOOLBARSTYLE = WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | TBSTYLE_TOOLTIPS | TBSTYLE_FLAT | BTNS_AUTOSIZE | CCS_NODIVIDER | CCS_TOP | WS_CLIPSIBLINGS;
...
bool ToolBar::init( HINSTANCE hInst, HWND hPere, toolBarStatusType type, ToolBarButtonUnit *buttonUnitArray, int arraySize)
{
	...
	_state = type;
	int iconSize = _dpiManager.scaleX(_state == TB_LARGE?32:16);
...
void ToolBar::reduce() 
{
	...
	int iconDpiDynamicalSize = _dpiManager.scaleX(16);
...
void ToolBar::enlarge()
{
	...
	int iconDpiDynamicalSize = _dpiManager.scaleX(32);
	_toolBarIcons.resizeIcon(iconDpiDynamicalSize);
...
void ToolBar::reset(bool create) 
{
	...
	if (!_hSelf)
	{
		_hSelf = ::CreateWindowEx(
		        ...
		        SetClassLongPtr(_hSelf, GCL_HBRBACKGROUND, (LONG_PTR)GetStockObject(GetSysColor(COLOR_BACKGROUND)));
                //                 SetWindowTheme(_hSelf, NULL, NULL); 
...
//         SetClassLongPtr(_hSelf, GCL_HBRBACKGROUND, (LONG_PTR)GetStockObject(WHITE_BRUSH));
